...
public class OptionsController extends GuiBaseController implements Initializable {
  ...
  private WindowWrapper winWrap = WindowWrapper.getWindowWrapper("options");
  private WinStateMachine winAPI = winWrap.getWindowSM();
  
  private WindowWrapper menuWinWrap = WindowWrapper.getWindowWrapper("first");
  private WinStateMachine menuWinAPI = menuWinWrap.getWindowSM();
  
  @FXML
  public void close() {
    try {
      winWrap.deleteWinWrap();
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  public void initialize(URL url, ResourceBundle resourceBundle) {
    ...
    ToggleSwitch btnFull = new ToggleSwitch(() -> { 
        menuWinAPI.setFullScreen();
      }, 
      () -> {
